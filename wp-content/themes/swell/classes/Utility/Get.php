...
trait Get {
	...
	public static function get_archive_data() {
		...
		if ( is_date() ) {
			...
			if ( $qv_day !== 0 ) {
				$ymd_name = sprintf( __( '%1$s年%2$s月%3$s日', 'swell' ), $qv_year, $qv_monthnum, $qv_day );
			} elseif ( $qv_monthnum !== 0 ) {
				$ymd_name = sprintf( __( '%1$s年%2$s月', 'swell' ), $qv_year, $qv_monthnum );
			} else {
				$ymd_name = sprintf( __( '%1$s年', 'swell' ), $qv_year );
			}
