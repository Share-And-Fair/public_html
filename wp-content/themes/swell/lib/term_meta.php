...
?>
	<tr class="swell_term_meta_title">
		<th colspan="2">
			<h2><?=esc_html__( 'SWELL設定', 'swell' )?></h2>
		</th>
	</tr>
	<tr class="form-field">
	...
	<tr class="form-field">
		...
		<td>
			...
			?>
			<?php if ( $parts_id ) : ?>
				<a href="<?=esc_url( admin_url( '/post.php?post=' . $parts_id . '&action=edit' ) )?>"><?=esc_html__( '編集ページへ', 'swell' )?></a>
			<?php endif; ?>
			<p class="description">
	...
	<tr class="form-field">
		...
		<td>
			...
			<?php
				...
				if ( $cta_id ) :
					?>
					<a href="<?=esc_url( admin_url( '/post.php?post=' . $cta_id . '&action=edit' ) )?>"><?=esc_html__( '編集ページへ', 'swell' )?></a>
			<?php endif; ?>
			<p class="description">
				<?=esc_html__( '※ ブログパーツのIDを半角で入力してください。', 'swell' )?><br>
